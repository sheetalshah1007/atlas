...
import static org.apache.atlas.repository.Constants.LABEL_NAME_DELIMITER;
...
public class EntityGraphMapper {
    ...
    public EntityGraphMapper(DeleteHandlerDelegate deleteDelegate, AtlasTypeRegistry typeRegistry, AtlasGraph graph,
            ...
            AtlasInstanceConverter instanceConverter, IFullTextMapper fullTextMapperV2, TaskManagement taskManagement) {
    ...
    public void addLabels(AtlasVertex vertex, Set<String> labels) throws AtlasBaseException {
        if (CollectionUtils.isNotEmpty(labels)) {
            final Set<String> existingLabels = getLabels(vertex);
        ...
        if (CollectionUtils.isNotEmpty(classifications)) {
            ...
            for (AtlasClassification c : classifications) {
                ...
                if (propagateTags && taskManagement != null && deferredActionEnabled) {
                    ...
                    createAndQueueTask(CLASSIFICATION_PROPAGATION_ADD, entityVertex, classificationVertex.getIdForDisplay(), classificationName, isImportInProgress);
                }
    ...
    public List<String> propagateClassification(String entityGuid, String classificationVertexId, String relationshipGuid) throws AtlasBaseException {
        try {
            ...
            List<String> impactedVerticesGuidsToLock = impactedVertices.stream().map(GraphHelper::getGuid).collect(Collectors.toList());
    ...
    public void deleteClassification(String entityGuid, String classificationName) throws AtlasBaseException {
        ...
        if (isPropagationEnabled(classificationVertex)) {
            if (taskManagement != null && deferredActionEnabled) {
                ...
                boolean pendingTaskExists = taskManagement.getPendingTasks().stream().anyMatch(x -> classificationHasPendingTask(x, classificationVertexId, entityGuid));
                ...
                createAndQueueTask(CLASSIFICATION_PROPAGATION_DELETE, entityVertex, classificationVertexId, classificationName, isImportInProgress);
    ...
    public void updateClassifications(EntityMutationContext context, String guid, List<AtlasClassification> classifications) throws AtlasBaseException {
        ...
        Set<AtlasVertex>          notificationVertices   = new HashSet<>(Collections.singleton(entityVertex));
        ...
        for (AtlasClassification classification : classifications) {
            ...
            if (updatedTagPropagation != null && taskManagement != null && deferredActionEnabled) {
                ...
                createAndQueueTask(propagationType, entityVertex, classificationVertex.getIdForDisplay(), classificationName, isImportInProgress);
    ...
    public List<String> deleteClassificationPropagation(String entityGuid, String classificationVertexId) throws AtlasBaseException {
        try {
            ...
            return propagatedEntities.stream().map(AtlasEntity::getGuid).collect(Collectors.toList());
        } catch (Exception e) {
            throw new AtlasBaseException(e);
        }
    }
    ...
    private void mapRelationshipAttributes(AtlasEntity entity, AtlasEntityType entityType, AtlasVertex vertex, EntityOperation op, EntityMutationContext context) throws AtlasBaseException {
        ...
        if (MapUtils.isNotEmpty(entity.getRelationshipAttributes())) {
            ...
            if (op.equals(CREATE)) {
                for (String attrName : entityType.getRelationshipAttributes().keySet()) {
                    ...
                    mapRelationshipAttribute(entityType, attrName, attrValue, vertex, op, context);
                }
            } else if (op.equals(UPDATE) || op.equals(PARTIAL_UPDATE)) {
    ...
    private void mapRelationshipAttribute(AtlasEntityType entityType, String attrName, Object attrValue, AtlasVertex vertex, EntityOperation op, EntityMutationContext context) throws AtlasBaseException {
        LOG.debug("==> mapRelationshipAttribute({}, {})", attrName, entityType.getTypeName());
        ...
        if (attrValue instanceof Collection) {
            ...
            LOG.warn("mapRelationshipAttribute: attribute {} on {}: Map-valued relationship attribute is not supported; use a list or set of references",
    ...
    private boolean isAttributeWithMultipleRelationshipTypes(Set<String> relationshipTypeNames) {
        return CollectionUtils.isNotEmpty(relationshipTypeNames) && relationshipTypeNames.size() > 1;
    }
        ...
        switch (ctx.getAttrType().getTypeCategory()) {
            ...
            case OBJECT_ID_TYPE: {
                ...
                if (currentEdge != null && !currentEdge.equals(newEdge)) {
                    ...
                    deleteDelegate.getHandler().deleteEdgeReference(currentEdge, ctx.getAttrType().getTypeCategory(), ctx.getAttribute().isOwnedRef(), true, ctx.getAttribute().getRelationshipEdgeDirection(), ctx.getReferringVertex());
                }
            ...
            case OBJECT_ID_TYPE:
                if (inverseEdge != null) {
                    if (!inverseEdge.equals(newEdge)) {
                        ...
                        deleteDelegate.getHandler().deleteEdgeReference(inverseEdge, inverseAttribute.getAttributeType().getTypeCategory(), inverseAttribute.isOwnedRef(), true, inverseVertex);
                    } else {
    ...
    private AtlasEdge createInverseReferenceUsingRelationship(EntityMutationContext context, AtlasAttribute inverseAttribute, AtlasEdge edge, Map<String, Object> relationshipAttributes) throws AtlasBaseException {
        ...
        if (inverseAttributeType instanceof AtlasEntityType) {
            ...
            if (entityType.hasRelationshipAttribute(inverseAttributeName)) {
                ...
                if (LOG.isDebugEnabled()) {
                    LOG.debug("No RelationshipDef defined between {} and {} on attribute: {}", inverseAttributeType, AtlasGraphUtilsV2.getTypeName(vertex), inverseAttributeName);
                }
        ...
        if (type instanceof AtlasEntityType) {
            ...
            if (entityType.hasRelationshipAttribute(attributeName)) {
                ...
                if (WARN_ON_NO_RELATIONSHIP || LOG.isDebugEnabled()) {
                    LOG.warn("No RelationshipDef defined between {} and {} on attribute: {}. This can lead to severe performance degradation.", getTypeName(entityVertex), getTypeName(attributeVertex), attributeName);
                }
    ...
    private Map<String, Object> removeUnusedMapEntries(AtlasAttribute attribute, AtlasVertex vertex, Map<String, Object> currentMap, Map<String, Object> newMap) throws AtlasBaseException {
        ...
        for (String currentKey : currentMap.keySet()) {
            ...
            if (!newMap.containsValue(currentEdge)) {
    ...
    private AtlasEdge updateEdge(AtlasAttributeDef attributeDef, Object value, AtlasEdge currentEdge, final AtlasVertex entityVertex) throws AtlasBaseException {
        LOG.debug("Updating entity reference {} for reference attribute {}", entityVertex, attributeDef.getName());
    ...
    private AtlasEdge updateRelationship(AtlasEdge currentEdge, final AtlasVertex parentEntityVertex, final AtlasVertex newEntityVertex, AtlasRelationshipEdgeDirection edgeDirection, Map<String, Object> relationshipAttributes) throws AtlasBaseException {
        if (LOG.isDebugEnabled()) {
            LOG.debug("Updating entity reference using relationship {} for reference attribute {}", getTypeName(newEntityVertex), parentEntityVertex);
        }
            ...
            if (isReference(entryType)) {
                ...
                if (CollectionUtils.isNotEmpty(edgesToRemove)) {
                    ...
                    for (AtlasEdge edge : edgesToRemove) {
                        boolean deleted = deleteDelegate.getHandler().deleteEdgeReference(edge, entryType.getTypeCategory(), attribute.isOwnedRef(), true, attribute.getRelationshipEdgeDirection(), entityVertex);
    ...
    private boolean classificationHasPendingTask(AtlasTask task, String classificationVertexId, String entityGuid) {
        if (task.getParameters() == null || task.getParameters().get(ClassificationTask.PARAM_CLASSIFICATION_VERTEX_ID) == null || task.getParameters().get(ClassificationTask.PARAM_ENTITY_GUID) == null) {
            return false;
        }
    ...
    private void activateEntityRelationships(AtlasVertex vertex, Set<String> relatedEntitiesGuids) {
        for (AtlasEdge edge : (Iterable<AtlasEdge>) vertex.getEdges(AtlasEdgeDirection.BOTH)) {
    ...
    private void addToUpdatedBusinessAttributes(Map<String, Map<String, Object>> updatedBusinessAttributes, AtlasBusinessAttribute bmAttribute, Object attrValue) {
        ...
        Map<String, Object> attributes = updatedBusinessAttributes.computeIfAbsent(bmName, k -> new HashMap<>());
